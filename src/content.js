...
// Build-time git dates (last-commit date per content file), generated by
// scripts/gen-dates.mjs before each build. Frontmatter `date` wins as the
// publish date; git date supplies the "last updated" fallback.
import gitDates from './generated/dates.js'
...
for (const [absPath, raw] of Object.entries(mdModules)) {
  ...
  const gitDate = gitDates[`${rel}.md`] || ''
  const date = meta.date || gitDate || ''
  // updated = git date when it differs from the declared publish date
  const updated = meta.date && gitDate && gitDate !== meta.date ? gitDate : ''
  posts.push({
    type: 'md',
    rel,
    url,
